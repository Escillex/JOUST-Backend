...
async function build(
  opts: { mail?: boolean; user?: Record<string, unknown> } = {},
) {
  ...
  const twoFactor = {
    checkCode: jest.fn(async (_id: string, code: string) =>
      code === '123456' ? { ok: true } : { ok: false, reason: 'invalid' },
    ),
    issueCode: jest.fn(async (_user: unknown, _kind: string) => ({
      sent: true,
    })),
  ...
  const google = {
    identify: jest.fn(async (c: string) => ({
      sub: c === 'good' ? user.googleId : 'someone-else',
    })),
  };
...
describe('account settings — which proof', () => {
  ...
  it('has nothing to ask for from a Google-only account on a site without mail, and refuses', async () => {
    ...
    expect(
      await codeOf(svc.changePassword('u1', { newPassword: 'another-one' })),
    ).toBe('NO_PROOF_AVAILABLE');
  });
...
describe('account settings — Google as proof', () => {
  it('is what a Google-only account on a mail-less site is asked for', async () => {
    const { svc, user } = await build({
      mail: false,
      user: { hashedPassword: null, googleId: 'g-1' },
    });
    expect(await svc.proofMethod(user)).toBe('google');
    expect(
      await codeOf(svc.changePassword('u1', { newPassword: 'first-one-1' })),
    ).toBe('GOOGLE_REQUIRED');
  });
  ...
  it('lets that account set a password — a Google account becoming an ordinary one', async () => {
    ...
    await svc.changePassword('u1', {
      googleCredential: 'good',
      newPassword: 'first-one-1',
    });
    expect(prisma.user.update.mock.calls[0][0].data).toMatchObject({
      hashedPassword: 'hashed:first-one-1',
    });
  });
    ...
    expect(
      await codeOf(
        svc.changePassword('u1', {
          googleCredential: 'someone-elses',
          newPassword: 'another-one',
        }),
      ),
    ).toBe('GOOGLE_MISMATCH');
    const plain = await build({ mail: false });
    expect(
      await codeOf(
        plain.svc.changePassword('u1', {
          googleCredential: 'good',
          newPassword: 'another-one',
        }),
      ),
    ).toBe('GOOGLE_NOT_LINKED');
  });
...
describe('account settings — password', () => {
  ...
  it('saves, clears a forced change, and forgets every other remembered browser but this one', async () => {
    ...
    const res = await svc.changePassword(
      'u1',
      { currentPassword: PASSWORD, newPassword: 'another-one' },
      'this-device',
    );
    const data = prisma.user.update.mock.calls[0][0].data;
    expect(data).toMatchObject({
      hashedPassword: 'hashed:another-one',
  ...
  it('ends every other session, and hands this browser the account to re-issue one from', async () => {
    ...
    const stamp = prisma.user.update.mock.calls[0][0].data
      .sessionsValidFrom as Date;
    ...
    expect(stamp.getTime()).toBeGreaterThanOrEqual(
...
describe('account settings — email', () => {
  ...
  it('refuses an address another account uses — before spending the code', async () => {
    ...
    expect(
      await codeOf(
        svc.changeEmail('u1', { code: '123456', email: 'taken@club.org' }),
      ),
    ).toBe('EMAIL_TAKEN');
  ...
  it('refuses an address that can never receive mail, on a site that sends it', async () => {
    ...
    expect(
      await codeOf(
        svc.changeEmail('u1', { code: '123456', email: 'me@example.com' }),
      ),
    ).toBe('EMAIL_UNROUTABLE');
  });

  it('saves it lower-cased and unverified, so the next code-gated sign-in proves the new inbox', async () => {
    const { svc, prisma } = await build();
    await svc.changeEmail('u1', {
      currentPassword: PASSWORD,
      email: ' New@Club.org ',
    });
...
describe('account settings — recovery codes and devices', () => {
  it('makes new recovery codes only with proof', async () => {
    ...
    expect(
      await codeOf(svc.newRecoveryCodes('u1', { currentPassword: 'nope' })),
    ).toBe('WRONG_PASSWORD');
    expect(twoFactor.generateRecoveryCodes).not.toHaveBeenCalled();
    expect(
      await svc.newRecoveryCodes('u1', { currentPassword: PASSWORD }),
    ).toEqual({ codes: ['NEW-1'] });
  });

  it('says so when a device is not on your account', async () => {
    const { svc } = await build();
    await expect(svc.forgetDevice('u1', 'someone-elses')).rejects.toThrow(
      'not on your account',
    );
  });
});
...
describe('account settings — deleting your own account', () => {
  it('needs the username typed out', async () => {
    ...
    expect(
      await codeOf(
        svc.deleteSelf('u1', { currentPassword: PASSWORD, confirm: 'caleb' }),
      ),
  ...
  it('refuses while you are in a live tournament — before spending the proof', async () => {
    ...
    prisma.tournamentParticipant.findMany.mockResolvedValueOnce([
      { tournamentId: 't1', tournament: { name: 'Winter Open' } },
    ]);
    expect(
      await codeOf(
        svc.deleteSelf('u1', { code: '123456', confirm: 'caleb-ruiz' }),
      ),
    ).toBe('ACTIVE_IN_LIVE_TOURNAMENT');
    expect(twoFactor.checkCode).not.toHaveBeenCalled();
  });

  it('deletes through the admin path, then logs it by name only', async () => {
    const { svc, auth, audit } = await build();
    await svc.deleteSelf('u1', {
      currentPassword: PASSWORD,
      confirm: 'Caleb-Ruiz',
    });
    ...
    expect(auth.deleteUser.mock.invocationCallOrder[0]).toBeLessThan(
    ...
    const prisma: any = {
      user: {
        findUnique: jest.fn(async (args: any) =>
          args.where.id === 'u1' ? { id: 'u1', username: 'old-name' } : null,
        ),
        findFirst: jest.fn(async () => null),
...
describe('mail is "set up"', () => {
  const settings = (v: Record<string, string>) => ({
    get: async (k: string) => v[k] ?? null,
  });
  it('only for SMTP with host, user and password — the console transport never delivers', async () => {
    expect(await new MailService(settings({}) as any).isConfigured()).toBe(
      false,
    );
    expect(
      await new MailService(
        settings({ MAIL_TRANSPORT: 'console' }) as any,
      ).isConfigured(),
    ).toBe(false);
    expect(
      await new MailService(
        settings({ MAIL_TRANSPORT: 'smtp', MAIL_HOST: 'h' }) as any,
      ).isConfigured(),
    ).toBe(false);
    expect(
      await new MailService(
