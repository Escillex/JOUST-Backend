...
export class ParticipantController {
  ...
  @Audit({
    ...
    describe: (c) => (c.self ? `Joined ${c.t}` : `Added ${c.target} to ${c.t}`),
  })
  @Post('join')
  ...
  @Audit({
    action: 'participant.add_guest',
    category: AC.PARTICIPANT,
    tournament: { param: 'tournamentId' },
    pick: ['username'],
    describe: (c) => `Added guest "${String(c.body.username ?? '')}" to ${c.t}`,
  })
  @Post('guest')
  ...
  @Audit({
    ...
    targetUser: { body: 'userId' },
    describe: (c) =>
      c.self ? `Left ${c.t}` : `Removed ${c.target} from ${c.t}`,
  })
  @Delete('leave')
  ...
  @Audit({
    action: 'participant.seed',
    category: AC.PARTICIPANT,
    tournament: { param: 'tournamentId' },
    targetUser: { param: 'userId' },
    pick: ['seed'],
    describe: (c) =>
      `Set ${c.target}'s seed to ${String(c.body.seed)} in ${c.t}`,
  })
  @Patch(':userId/seed')
  @UseGuards(JwtAuthGuard, RolesGuard, TournamentAccessGuard)
  @Roles(Role.ORGANIZER, Role.ADMIN)
  ...
  @Audit({
    action: 'participant.forfeit',
    category: AC.PARTICIPANT,
    tournament: { param: 'tournamentId' },
    targetUser: { param: 'userId' },
    describe: (c) => `Forfeited ${c.target} in ${c.t}`,
  })
  @Post(':userId/forfeit')
  @UseGuards(JwtAuthGuard, RolesGuard, TournamentAccessGuard)
  @Roles(Role.ORGANIZER, Role.ADMIN)
  ...
  @Audit({
    action: 'participant.replace',
    category: AC.PARTICIPANT,
    tournament: { param: 'tournamentId' },
    targetUser: { param: 'userId' },
    pick: ['substituteUserId', 'guestName'],
    describe: (c) =>
      `Replaced ${c.target} in ${c.t}${c.body.guestName ? ` with guest "${String(c.body.guestName)}"` : ' with another player'}`,
  })
  @Post(':userId/replace')
  @UseGuards(JwtAuthGuard, RolesGuard, TournamentAccessGuard)
  @Roles(Role.ORGANIZER, Role.ADMIN)
