...
function TournamentViewContent() {
  ...
  const [editFormatConfig, setEditFormatConfig] = useState<FormatConfig>({});
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (meRes.ok) {
        ...
        const allowed = meData.roles?.some((r: string) => ["PLAYER", "ORGANIZER", "ADMIN"].includes(r));
        if (allowed) {
  ...
  const handleUpdateTournament = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { setMessage("Updated!"); setIsEditing(false); fetchData(); }
      else { const err = await res.json(); setMessage(`Error: ${err.message || "Update failed"}`); }
    } catch { setMessage("Error: Update failed"); }
  };

  const updateEditFormatConfig = (key: keyof FormatConfig, value: any) => {
    setEditFormatConfig(prev => ({ ...prev, [key]: value }));
  };
  ...
  const handleJoin = async (userId: string) => {
    try {
      ...
      if (res.ok) { setMessage("Joined!"); fetchData(); return true; }
      else {
        const err = await res.json();
        if (err.message?.includes("full")) alert(err.message);
        setMessage(err.message || "Join failed");
        return false;
      }
    } catch { setMessage("Join failed"); return false; }
  };
  ...
  const handleFillWithGuests = async () => {
    ...
    try {
      ...
      await fetchData();
    } catch { setMessage("Error during bulk addition"); await fetchData(); }
    finally { setLoading(false); }
  };
  ...
  const handleStartTournament = async () => {
    ...
    try {
      ...
      else { const err = await res.json(); setMessage(`Error: ${err.message || "Start failed"}`); }
  ...
  const getTournamentChampion = (tournament: Tournament | null) => {
    if (!tournament) return null;
    if (tournament.winner) return tournament.winner;
    const sortedRounds = [...(tournament.rounds || [])].sort((a, b) => b.roundNumber - a.roundNumber);
    return sortedRounds[0]?.matches.find(m => m.status === "COMPLETED" && m.winner)?.winner ?? null;
  };

  const champion = getTournamentChampion(tournament);
  const championName = champion?.guestName || champion?.username;
      ...
      <div className="max-w-6xl mx-auto w-full p-2 md:p-6 flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[1.1px] text-[#838383] border-b-2 border-[#2F2F2F] pb-2">
          ...
          <div className="flex gap-4">
            <span className="text-[#52B946]">{user?.guestName || user?.username || "WARRIOR"}</span>
            <button onClick={handleSignOut} className="hover:text-red-500 transition-colors">Exit</button>
          </div>
        </div>

        {/* Hero Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 p-8 rounded-[20px] shadow-lg" style={{ background: "linear-gradient(180deg, #2A2A2A 0%, #212121 100%)", borderTop: "5px solid #3E9434" }}>
          <div>
            <h1 className="text-4xl md:text-5xl font-black text-white tracking-[1.1px] uppercase leading-none">{tournament.name}</h1>
            <div className="flex items-center gap-3 mt-4">
              ...
              {championName && (
                <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-[10px] border border-[#52B946]/30">
                  ...
                  <span className="text-[10px] font-black text-white uppercase tracking-[1.1px]">{championName}</span>
                </div>
              )}
            </div>
          </div>
          <div className="flex gap-2 w-full md:w-auto">
            <button onClick={() => router.push(`/join_tournament/bracket_view?id=${tournamentId}`)} className="flex-1 md:px-8 h-12 bg-transparent border-2 border-[#52B946] text-[#52B946] rounded-[10px] text-[12px] font-black uppercase tracking-[1.1px] hover:bg-[#52B946] hover:text-white transition-all">Brackets</button>
            {isOrganizerOrAdmin && tournament.status === "OPEN" && (
              <button onClick={handleStartTournament} className="flex-1 md:px-8 h-12 bg-[#52B946] text-white rounded-[10px] text-[12px] font-black uppercase tracking-[1.1px] hover:bg-[#3E9434] transition-all">Start</button>
            )}
          </div>
        </div>

        {message && (
          <div className={`px-4 py-2 border text-[10px] font-black uppercase text-center rounded-[5px] tracking-[1.1px] ${message.startsWith("Error") ? "bg-red-500/10 border-red-500/30 text-red-500" : "bg-[#52B946]/10 border-[#52B946]/30 text-[#6FFF5E]"}`}>
            {message}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          {/* Participants */}
          <div className="lg:col-span-8 rounded-[20px] p-6 shadow-lg" style={{ background: "linear-gradient(180deg, #2A2A2A 0%, #212121 100%)" }}>
            <div className="flex justify-between items-center mb-6 border-b-2 border-[#2F2F2F] pb-4">
              ...
              {!isOrganizerOrAdmin && tournament.status === "OPEN" && !tournament.participants.some(p => p.userId === user?.sub) && (
                <button onClick={() => user && handleJoin(user.sub)} className="text-sm font-black text-[#52B946] hover:text-[#3E9434] tracking-[1.1px] uppercase underline">Join Now</button>
              )}
            </div>
                ...
                tournament.participants.map((p, idx) => (
                  <div key={p.id} className="flex items-center justify-between px-4 py-3 rounded-[10px]" style={{ background: "#101010" }}>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-white uppercase tracking-[1.1px] truncate max-w-[150px]">{p.user.guestName || p.user.username || "TBD"}</span>
                    </div>
              ...
              {isEditing ? (
                <form onSubmit={handleUpdateTournament} className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="h-10 rounded-[5px] flex items-center px-3" style={{ background: "#101010" }}>
                  ...
                  <div className="flex items-center gap-2">
                    <input type="checkbox" checked={editIsPrivate} onChange={e => setEditIsPrivate(e.target.checked)} className="accent-[#52B946]" id="editPrivate" />
                    <label htmlFor="editPrivate" className="text-[10px] font-black text-[#838383] uppercase tracking-[1.1px]">Private Tournament</label>
                  </div>

                  {/* ── Format Selector (compact variant for sidebar) ── */}
                  <div className="flex flex-col gap-1">
                    <label className="text-[10px] font-black text-[#838383] uppercase tracking-[1.1px]">Format</label>
                    <FormatSelector
                      value={editFormat}
                      onChange={(f) => setEditFormat(f)}
                      formatConfig={editFormatConfig}
                      onConfigChange={updateEditFormatConfig}
                      options={formatOptions}
                      variant="compact"
                    />
                  </div>

                  <button type="submit" className="h-11 bg-[#52B946] text-white font-black text-[12px] uppercase rounded-[10px] tracking-[1.1px] hover:bg-[#3E9434] transition-colors">
                    Update
                  </button>
                </form>
              ) : (
                ...
                <div className="flex flex-col gap-6">
                  <div>
                    ...
                    <div className="flex gap-2">
                      <div className="flex-1 h-10 rounded-[5px] flex items-center px-3" style={{ background: "#101010" }}>
                      ...
                      <button onClick={handleAddGuest} className="w-10 h-10 rounded-[5px] bg-transparent border-2 border-[#52B946] text-[#52B946] font-black text-xl hover:bg-[#52B946] hover:text-white transition-all">+</button>
                    </div>
                  </div>
                  <div>
