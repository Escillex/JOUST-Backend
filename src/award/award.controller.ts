...
const ART_UPLOAD = FileInterceptor('image', {
...
export class AwardCatalogController {
  ...
  @Audit({
    ...
    pick: ['name', 'kind'],
    describe: (c) =>
      `Created the ${String(c.body.kind ?? 'award').toLowerCase()} "${String(c.body.name ?? '')}"`,
  })
  @Post()
  ...
  @Audit({
    action: 'award.update',
    category: AC.AWARD,
    subject: { model: 'award', param: 'id' },
    pick: ['archived'],
    describe: (c) =>
      c.body.archived === true
        ? `Archived the award "${c.subject}"`
        : c.body.archived === false
          ? `Unarchived the award "${c.subject}"`
          : `Edited the award "${c.subject}" (${c.fields.join(', ')})`,
  })
  @Patch(':id')
  ...
  @Audit({
    ...
    category: AC.AWARD,
    subject: { model: 'award', param: 'id' },
    describe: (c) => `Replaced the artwork of "${c.subject}"`,
  })
  @Post(':id/image')
  @UseInterceptors(ART_UPLOAD)
  replaceImage(
  ...
  @Audit({
    ...
    category: AC.AWARD,
    subject: { model: 'award', param: 'id' },
    describe: (c) => `Deleted the award "${c.subject}"`,
  })
  @Delete(':id')
...
export class AwardGrantController {
  ...
  @Audit({
    ...
    subject: { model: 'award', body: 'awardId' },
    pick: ['awardId', 'note'],
    describe: (c) => `Gave "${c.subject}" to ${c.target}`,
  })
  @Post()
  grant(
  ...
  @Audit({
    ...
    category: AC.AWARD,
    targetUser: { param: 'userId' },
    subject: { model: 'userAward', param: 'grantId' },
    describe: (c) => `Revoked "${c.subject}" from ${c.target}`,
  })
  @Delete(':grantId')
  revoke(
    @Param('userId', ParseUUIDPipe) userId: string,
