...
# Deployment name = docker compose project + volume prefix. Give a DISTINCT name
# to stand up a second, isolated instance on the same box (its own containers,
# network, and pgdata volume) instead of clobbering the default 'joust' one.
PROJECT=$(existing_env COMPOSE_PROJECT_NAME)
ask    PROJECT "Deployment name (distinct = isolated second instance)" "${PROJECT:-joust}"
...
# Postgres host port (127.0.0.1-bound). Bump it for a second instance so it does
# not fight the default 5433 for the host binding.
DB_PORT=$(existing_env DB_PORT)
ask    DB_PORT "Postgres host port" "${DB_PORT:-5433}"
