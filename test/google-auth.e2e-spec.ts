import {
  GoogleAuthService,
  GoogleIdentity,
} from '../src/auth/google-auth.service';
...
function build(
  ...
  const settingsSvc = {
    get: jest.fn(
      async (k: string) => (settings as Record<string, string>)[k] ?? null,
    ),
    getBoolean: jest.fn(
      async (k: string) => (settings as Record<string, string>)[k] === 'true',
    ),
  };
  const auth = {
    startSession: jest.fn(async (u: any) => ({
      token: 't',
      roles: u.roles,
      userId: u.id,
    })),
...
describe('Google sign-in configuration', () => {
  it('offers nothing until it is enabled AND has a Client ID', async () => {
    ...
    expect(
      await build({ settings: { GOOGLE_CLIENT_ID: '' } }).svc.providers(),
    ).toEqual({ google: { enabled: false, clientId: null } });
    expect(await build().svc.providers()).toEqual({
      google: {
        enabled: true,
  ...
  it('refuses to verify anything while disabled', async () => {
    const { svc, verify } = build({
      settings: { GOOGLE_SIGNIN_ENABLED: 'false' },
    });
    await expect(svc.signIn('tok', res)).rejects.toMatchObject({
      response: { code: 'GOOGLE_SIGNIN_DISABLED' },
    });
  ...
  it('verifies against the Client ID from Settings — whatever this deployment configured', async () => {
    const { svc, verify } = build({
      settings: {
        GOOGLE_CLIENT_ID: 'school-deployment.apps.googleusercontent.com',
      },
    });
    await svc.signIn('tok', res);
    expect(verify).toHaveBeenCalledWith(
      'tok',
      'school-deployment.apps.googleusercontent.com',
    );
  });
  ...
  it('turns a bad or forged token into a 401, not a 500', async () => {
    ...
    await expect(svc.signIn('forged', res)).rejects.toThrow(
      /could not confirm/i,
    );
  });
  ...
  it('enforces the allowed Workspace domain when one is set', async () => {
    const outsider = build({
      settings: { GOOGLE_ALLOWED_DOMAIN: 'school.edu' },
      identity: ident({ hd: null }),
    });
    await expect(outsider.svc.signIn('tok', res)).rejects.toMatchObject({
      response: { code: 'GOOGLE_DOMAIN_NOT_ALLOWED' },
    });
    const member = build({
      settings: { GOOGLE_ALLOWED_DOMAIN: 'School.edu' },
      identity: ident({ hd: 'school.edu' }),
    });
    await expect(member.svc.signIn('tok', res)).resolves.toMatchObject({
      via: 'google',
    });
  });
});
...
describe('account matching', () => {
  ...
  it('REFUSES an email match on an account that has a password — no takeover by email', async () => {
    ...
    await expect(svc.signIn('tok', res)).rejects.toMatchObject({
      response: { code: 'GOOGLE_EMAIL_HAS_PASSWORD' },
    });
  ...
  it('links an email match that has no password, and marks the address verified', async () => {
    ...
    await expect(svc.signIn('tok', res)).resolves.toMatchObject({
      linked: true,
    });
  ...
  it('creates a verified, password-less account with a valid unique username', async () => {
    const { svc, prisma } = build({
      identity: ident({ email: 'p.scholes+utd@gmail.com' }),
    });
...
describe('connecting and disconnecting', () => {
  it("won't connect a Google account that already belongs to someone else", async () => {
    ...
    await expect(svc.link('me', 'tok')).rejects.toMatchObject({
      response: { code: 'GOOGLE_ALREADY_LINKED' },
    });
  });
