...
import { BuildSettingsDto, ReviewBuildDto, SubmitBuildDto } from './dto/content.dto';

type Viewer = JwtPayload | null | undefined;
const viewerId = (v: Viewer) => (v ? (v as { id?: string; sub?: string }).id ?? (v as { sub?: string }).sub : undefined);

const personSelect = { id: true, username: true, displayName: true, slug: true, isGuest: true } as const;
...
export class BuildService {
  ...
  private async rules(tournamentId: string): Promise<TournamentBuildRules & { name: string }> {
  ...
  static visibleToPublic(
    t: Pick<TournamentBuildRules, 'status' | 'buildsRequired' | 'buildVisibility'>,
  ...
  async list(tournamentId: string, viewer: Viewer) {
    ...
    const staff = (await checkTournamentAccess(this.prisma, tournamentId, viewer)) === 'ALLOWED';
    ...
    const visible = rows
      .filter((b) => staff || b.userId === me || BuildService.visibleToPublic(t, b))
      .map((b) =>
        staff || b.userId === me ? b : { ...b, reviewNote: null, reviewedById: null },
      );
    ...
    if (staff) {
      ...
      entrants = parts.map((p) => ({
        ...
        status: byUser.get(p.userId)?.status ?? (p.user.isGuest ? 'GUEST' : 'MISSING'),
      }));
    }
  ...
  async submit(tournamentId: string, userId: string, dto: SubmitBuildDto, file?: Express.Multer.File) {
    const t = await this.rules(tournamentId);
    if (this.isLocked(t)) {
    ...
    if (!entry || entry.status !== ParticipantStatus.ACTIVE) {
      throw new ForbiddenException('Only an active entrant can submit a build for this tournament.');
    }
    ...
    const saved = existing
      ? await this.prisma.tournamentBuild.update({ where: { id: existing.id }, data })
      : await this.prisma.tournamentBuild.create({ data: { tournamentId, userId, ...data } });
  ...
  async withdraw(tournamentId: string, userId: string) {
    const t = await this.rules(tournamentId);
    if (this.isLocked(t)) {
      throw new ForbiddenException({ code: 'BUILDS_LOCKED', message: 'Builds are locked for this tournament.' });
    }
    const existing = await this.prisma.tournamentBuild.findFirst({
      where: { tournamentId, userId, removedAt: null },
    });
    if (!existing) throw new NotFoundException('You have not submitted a build.');
  ...
  async review(tournamentId: string, buildId: string, reviewerId: string, dto: ReviewBuildDto) {
    const t = await this.rules(tournamentId);
    ...
    const updated = await this.prisma.tournamentBuild.update({
      ...
      data: {
        ...
        reviewedAt: new Date(),
        reviewNote: dto.decision === 'REJECTED' ? dto.note!.trim() : dto.note?.trim() || null,
      },
    });
  ...
  async updateSettings(tournamentId: string, dto: BuildSettingsDto) {
    const t = await this.rules(tournamentId);
    const started = t.status === TournamentStatus.ONGOING || t.status === TournamentStatus.COMPLETED;
    if (started && (dto.buildsRequired !== undefined || dto.buildsLockAtStart !== undefined)) {
    ...
    return this.prisma.tournament.update({
      where: { id: tournamentId },
      data: dto,
      select: { buildsRequired: true, buildVisibility: true, buildsLockAtStart: true },
    });
  }
 ...
 */
export async function assertBuildsReady(prisma: PrismaService, tournamentId: string) {
  ...
  const parts = await prisma.tournamentParticipant.findMany({
    where: { tournamentId, status: ParticipantStatus.ACTIVE, user: { isGuest: false } },
  ...
  const name = (p: (typeof parts)[number]) => p.user.displayName || p.user.username || 'Player';

  const missing = parts.filter((p) => !status.has(p.userId)).map(name);
  const pending = parts
    .filter((p) => status.has(p.userId) && status.get(p.userId) !== BuildStatus.APPROVED)
    .map(name);
