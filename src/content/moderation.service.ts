...
const nameOf = (
  u: { username: string | null; displayName: string | null } | null | undefined,
...
export class ModerationService {
  ...
  private async load(type: TargetType, id: string) {
    if (type === 'GALLERY_IMAGE') {
      ...
      return g
        ? {
            ...
            ownerId: g.userId,
            owner: g.user,
            label: `gallery image for ${g.game.name}`,
          }
        : null;
    }
    ...
    return b
      ? {
          ...
          ownerId: b.userId,
          owner: b.user,
          label: `build for ${b.tournament.name}`,
  ...
  async report(reporter: Actor, dto: ReportDto) {
    ...
    const fromStaff =
      roles.includes(Role.ORGANIZER) || roles.includes(Role.ADMIN);
    try {
  ...
  async openCount() {
    const [g, b] = await Promise.all([
      this.prisma.galleryImage.count({
        where: {
          removedAt: null,
          reports: { some: { status: ReportStatus.OPEN } },
        },
      }),
      this.prisma.tournamentBuild.count({
        where: {
          removedAt: null,
          reports: { some: { status: ReportStatus.OPEN } },
        },
  ...
  async queue(view: 'open' | 'removed') {
    ...
    const removerIds = [...gallery, ...builds]
      .map((r) => r.removedById)
    ...
    const hold = (removedAt: Date | null) =>
      removedAt
        ? {
            ...
            purgeAt: new Date(removedAt.getTime() + HOLD_MS).toISOString(),
          }
        : {};
    ...
    const items = [
      ...gallery.map((g) => ({
        ...
        removedByName: g.removedById
          ? (removers.get(g.removedById) ?? null)
          : null,
        ...hold(g.removedAt),
      })),
      ...builds.map((b) => ({
        ...
        removedByName: b.removedById
          ? (removers.get(b.removedById) ?? null)
          : null,
        ...hold(b.removedAt),
      })),
    ];
    ...
    return items.sort((a, b) => {
      ...
      return (a.reports[0]?.createdAt ?? '').localeCompare(
  ...
  async remove(admin: Actor, dto: ModerationRemoveDto) {
    ...
    if (target.row.removedAt)
      throw new ConflictException('That item has already been removed.');

    const data = {
      removedAt: new Date(),
      removedById: actorId(admin),
    ...
    await this.prisma.$transaction([
      dto.targetType === 'GALLERY_IMAGE'
        ...
        : this.prisma.tournamentBuild.update({
    ...
    await this.notifications.notify({
      ...
      link:
        dto.targetType === 'GALLERY_IMAGE'
          ? '/profile/edit'
          : `/tournaments/${'tournament' in target ? target.tournament?.id : ''}`,
    });
    return {
      message: `Removed. It can be restored for ${REMOVAL_HOLD_DAYS} days.`,
    };
  }
  ...
  async dismiss(admin: Actor, dto: ModerationTargetDto) {
    ...
    if (count === 0)
      throw new NotFoundException('There are no open reports on that item.');
  ...
  async restore(dto: ModerationTargetDto) {
    const target = await this.load(dto.targetType, dto.targetId);
    if (!target || !target.row.removedAt)
      throw new NotFoundException('That item is not in the removed list.');
    const data = { removedAt: null, removedById: null, removalReason: null };
    try {
      if (dto.targetType === 'GALLERY_IMAGE')
      ...
      if (
        ...
        throw new ConflictException({
          code: 'SLOT_TAKEN',
          message:
            'The owner has uploaded a replacement since, so this one cannot be restored alongside it.',
        });
      }
      throw err;
    }
  ...
  async purgeExpired(now = new Date()): Promise<number> {
    ...
    if (gallery.length)
      await this.prisma.galleryImage.deleteMany({
        where: { id: { in: gallery.map((g) => g.id) } },
      });
    if (builds.length)
      await this.prisma.tournamentBuild.deleteMany({
        where: { id: { in: builds.map((b) => b.id) } },
      });
    for (const f of [...gallery, ...builds])
      if (f.imageUrl) await this.images.deleteFile(f.imageUrl);
    const n = gallery.length + builds.length;
    if (n)
      this.logger.log(
        `Purged ${n} removed item(s) past their ${REMOVAL_HOLD_DAYS}-day hold.`,
      );
    return n;
  }
}
