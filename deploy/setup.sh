...
render_host_cloudflared() {
	...
	# resolve name → UUID via `tunnel info` when possible; else fall back to the
	# single credentials file present.
	have cloudflared && id=$(cloudflared tunnel info "$TUNNEL_NAME" 2>/dev/null | grep -oiE '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}' | head -1)
	if [ -z "$id" ]; then
		cred=$(find "$cfdir" -maxdepth 1 -name '*.json' 2>/dev/null | head -1)
		[ -z "$cred" ] && die "no tunnel credentials in $cfdir — run: cloudflared tunnel login && cloudflared tunnel create $TUNNEL_NAME"
